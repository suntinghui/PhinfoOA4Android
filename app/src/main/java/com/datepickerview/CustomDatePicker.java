...
public class CustomDatePicker {
    ...
    public interface ResultHandler {
        void handle(String time,int yyyy,int MM,int dd,int HH,int mm);
    }
    ...
    private boolean bShowSpecificDay=true,bShowSpecificTime=true;
    ...
    private void initView() {
        ...
        tv_select.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                cal.setTime(selectedCalender.getTime());  
                int year = cal.get(Calendar.YEAR);
                int month = cal.get(Calendar.MONTH);
                int day = cal.get(Calendar.DAY_OF_MONTH);
                int hour = cal.get(Calendar.HOUR_OF_DAY);
                int min = cal.get(Calendar.MINUTE);
                String date = String.format("%04d-%02d-%02d %02d:%02d",year,month+1,day,hour,min);
                if(!bShowSpecificDay){
                	date = String.format("%04d-%02d",year,month+1);
                }else if(!bShowSpecificTime){
                	date = String.format("%04d-%02d-%02d",year,month+1,day);
                }
                handler.handle(date,year,month+1,day,hour,min);
                datePickerDialog.dismiss();
            }
        });
    }
    ...
    public void show(String time) {
        if (canAccess) {
        	if(!bShowSpecificDay&&isValidDate(time, "yyyy-MM")){
        		time= String.format("%s-%02d",time,01);
        	}
