...
//				if (mState == State.RELEASE_TO_REFRESH
//						&& (mMode == Mode.REFROTATE || mMode == Mode.FLIP_ROTATE)
//						&& (null != mOnRefreshListener || null != mOnRefreshListener2)) {
//					setState(State.REFRESHING, true);
//					return true;
//				}
	...
	public void setDefaultUpdateLabel() {
		if (mCurrentMode == Mode.PULL_FROM_END
				&& (mMode == Mode.FLIP_ROTATE || mMode == Mode.REFROTATE)) {
			getLoadingLayoutProxy().setLastUpdatedLabel("");
		}else{
			String DATE_FORMAT_STR = "yyyy年MM月dd日 HH:mm";
			String s = getResources()
					.getString(R.string.p2refresh_refresh_lasttime)
					+ new SimpleDateFormat(DATE_FORMAT_STR, Locale.CHINA)
							.format(new Date());
			getLoadingLayoutProxy().setLastUpdatedLabel(s);
		}
	...
	protected final LoadingLayout createLoadingLayout(Context context,
	...
	private final void createDefaultLoadingLayout(Context context, Mode mode,
			TypedArray a) {
		mHeaderLayout = mLoadingAnimationStyle.createTopLoadingLayout(context,
				mode, getPullToRefreshScrollDirection(), a);
		// �?????
		mFooterLayout = mLoadingAnimationStyle.createBootomLoadingLayout(
				context, Mode.PULL_FROM_END, getPullToRefreshScrollDirection(),
				a);
		mFooterLayout.setVisibility(View.INVISIBLE);
		// if (mMode == Mode.REFROTATE || mMode == Mode.FLIP_ROTATE) {
		// mFooterLayout =
		// mLoadingAnimationStyle.createBootomLoadingLayout(context,
		// Mode.REFROTATE,
		// getPullToRefreshScrollDirection(),a);
		// mFooterLayout.setVisibility(View.INVISIBLE);
		// } else {
		// mFooterLayout =
		// mLoadingAnimationStyle.createBootomLoadingLayout(context,
	...
	public static enum Mode {
		 ...
		 */
		MANUAL_REFRESH_ONLY(0x4), FLIP_ROTATE(0x5), REFROTATE(0x6);
		...
		public boolean showFooterLoadingLayout() {
			return this == PULL_FROM_END || this == BOTH
					|| this == MANUAL_REFRESH_ONLY || this == FLIP_ROTATE
					|| this == REFROTATE;
		}
